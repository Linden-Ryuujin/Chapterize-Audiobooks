...
def is_number_word(line):
    ...
    number = False
    for iNumber in numbers:
        if iNumber in line:
            line = line.replace(iNumber, " ")
            number = True

    #con.print(f"result: {line.isspace() and number}")
    return (line.isspace() and number)

def parse_timecodes(content: list, numbers = False) -> list[dict]:
    """
    ...
    for i, line in enumerate(content):
        if (
                ...
                ('chapter' in content[i+1] or
                 ...
                 'epilogue' in content[i+1] or
                 is_number_word(content[i+1]))
        ):
            if start_regexp := re.search(r'\d\d:\d\d:\d\d,\d+(?=\s-)', line, flags=0):
                ...
                elif (not numbers and 'chapter' in content[i+1]):
                    chapter_type = f'Chapter {counter}'
                    counter += 1
                elif (numbers and is_number_word(content[i+1])):
                    chapter_type = f'Number {content[i+1][:-1]}'
...
def print_timecodes(timecodes_file, audiobook_file, numbers = False):
    with open(timecodes_file, 'r') as fp:
        file_lines = fp.readlines()
    con.rule("[cyan]Parse Timecodes[/cyan]")
    print("\n")
    timecodes = parse_timecodes(file_lines, numbers)
...
def main():
    ...
    print_timecodes(timecodes_file, audiobook_file)
